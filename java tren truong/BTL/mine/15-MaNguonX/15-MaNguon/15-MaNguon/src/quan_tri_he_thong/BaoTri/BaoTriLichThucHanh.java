...
public class BaoTriLichThucHanh {
		...
		do {
			...
			if(ngay <= 0) {
                ...
                int chonCa;
                        ...
                        System.out.print("--->Nhập ca thực hành:");
                ...
                String maPhong;
		do {
			System.out.print("--->Nhập mã phòng đăng kí thực hành:");
                        maPhong = sc.nextLine();
			
			if(maPhong.length() == 0) {
				System.out.println("Tên mã không được để trống,mời nhập lại....!!!!");
			}
    ...
    private static void suaLichThucHanh() {
		...
		do {
			...
			case 1:
				...
				listt.get(index).setNgay(ngayms);
				...
				break;
                        ...
                        case 2:
                            String caThucHanh;
				do {
					System.out.print("--->Nhập ca thực hành mới:");
                                        caThucHanh = sc.nextLine();
					 
				if(caThucHanh.length() == 0)
					System.out.println("Ca thực hành không được để trống ,mời nhập lại....!!!! ");
				}while(caThucHanh.length() == 0);
				
				listt.get(index).setCaThucHanh(caThucHanh);
    ...
    private static void hienThiLichThucHanh() {
		...
		System.out.println();
		System.out.printf("   %-10s %-20s %-10s %12 %-12s %-30s %-30s %n","ngay","caThucHanh","khuNha","maPhong","maLop","tenGV","tenMonHoc");  
		for(LichThucHanh lth : listt){
                    lth.hienThilich();
                }
	}
